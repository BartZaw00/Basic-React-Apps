...
function App() {
  ...
  function handleClick(option) {
    ...
    if (option === 2) {
      // option - right
      setIndex((prevIndex) => {
        if (prevIndex === reviewsArray.length - 1) return 0;
        return prevIndex + 1;
      });
    }
    if (option === 3) {
      // option - surprise
      let randomNumber = Math.floor(Math.random() * reviewsArray.length);
      if (randomNumber === index)
        while (randomNumber === index)
          randomNumber = Math.floor(Math.random() * reviewsArray.length);
  ...
  return (
    <>
      <main id="container">
        ...
        <div className="box">
          <Profile {...reviewsArray[index]} onChange={handleClick} />
          <div>
            <button
              className="btn left"
          ...
          <div className="btn-surprise-me" onClick={() => handleClick(3)}>
            Surprise Me
          </div>
        </div>
      </main>
      <footer>
        <a id="credits" href="https://github.com/BartZaw00" target="_blank">
          Created By <span>Bartosz Zawadka</span>
        </a>
      </footer>
    </>
  );
